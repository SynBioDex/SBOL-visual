...
\begin{enumerate}
\item A molecular species glyph MUST NOT contact any nucleic acid backbone with any part of its bounding box, \twotwozero{unless the diagram is showing that the molecular species binds to the nucleic acid.}

\item \twotwozero{If binding is being shown, the molecular species glyph SHOULD contact an appropriate sequence feature glyph (e.g., for a binding site or restriction site) and not the backbone itself (see example in \ref{apdx:sym:species} for glyph Complex).}
