...
\packageVersion{Version 3.0-alpha}
\packageVersionDate{TBD}

% BBF RFC \rfcnum{}: 
\title{Synthetic Biology Open \\Language Visual (SBOL Visual) Version~3.0-alpha}
...
James McLaughlin & \emph{EMBL-EBI, UK}\\
...
\bf{and also any others} & \bf{who contributed since 2.3 or otherwise should be authors} \\
\end{tabular}
%\end{small}
}
...
\todo[inline]{Go through contributions since 2.3 release and see if there are other authors to include}
\todo[inline]{Check if all significant Component stuff can be represented in SBOL Visual 3. Constraints? Interfaces?}
\todo[inline]{Need to figure out where the description of parametric goes. Is it part of the Glyphs section, or is it its own section?}
\todo[inline]{Change glyph directory names to match glyph categories, rather than SBOL 2 terms}
\todo[inline]{Clean shmoos out of diagrams}
