...
Examples are provided in~\ref{exa:4d}.
	...
	(e) composite edges representing two interactions: CRISPR complex formation with Cas9m from two sources, which then represses a promoter.
...
\twotwozero{
	\begin{figure}[h!]
	\centering
	\subfigure[MAY]{\includegraphics[scale=0.4]{figures/examples/4e-stimulate.pdf}}
	\subfigure[MAY]{\includegraphics[scale=0.4]{figures/examples/4e-nodeless.pdf}}
	\subfigure[MUST NOT]{\includegraphics[scale=0.4]{figures/examples/4e-multiarrow.pdf}}
	\subfigure[MUST NOT]{\includegraphics[scale=0.4]{figures/examples/4e-onearrow.pdf}}
	\caption{Examples of edges going into process nodes: 
	(a) gRNA stimulating the Cas9m repression process (an alternative Figure~\ref{exa:4d}(e) representation),  (b) the same but omitting the process node.
