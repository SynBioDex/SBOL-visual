...
In terms of the SBOL 2 data model, the line represents a \sbol{Module} included within the \sbol{ModuleDefinition} represented by the surrounding diagram, and boundary-crossing elements defined \sbol{MapsTo} relationships.
Note that the internals of a module need not be shown: some details can be omitted or a module can even be a ``black box'' with no structure at all being shown.
...
\item The boundary of module SHOULD should be a rectangle or rounded rectangle. Boundary sides SHOULD be oriented vertically and horizontally.
	...
	\begin{figure}[h!]
	\centering
	\subfigure[SHOULD]{rectangle}
	\subfigure[SHOULD]{round rectangle}
	\subfigure[SHOULD NOT]{tilted rectangle}
	\subfigure[SHOULD NOT]{blobby mess}
	\caption{}
	\label{exa:moduleA}
	\end{figure}

\item An undirected edge (i.e., having no ``arrow head'') crossing the boundary of a module represents an identity mapping between the 

\item Glyphs for sequence features and molecular species MUST NOT intersect with the boundary of a module.
\item A nucleic acid backbone MAY cross the boundary of a module. This represents...
\item An interaction edge MAY cross the boundary of a module. This represents...

\item Rectangles MAY be drawn on the outside of the module boundary to represent input/output ports.
...
If both a port and its associated component (sequence feature or module species) are present in a diagram, then they MUST be visually connected, either explicitly by means of a mapping or implicitly by an interaction that passes through the port rectangle.
Likewise, mappings and interactions with the associated component MUST NOT cross the boundary except through the port.




 if a line or arrow connects to the outside of a port, it should be interpreted as being directly connected to any glyph inside the module that is connected to the inside of the same port. If arrows connect to both the outside and inside of a port, then they MUST have consistent directions. A port MUST NOT be connected to more than one glyph outside of the module, unless all of the glyphs it connects to represent the same entity.
